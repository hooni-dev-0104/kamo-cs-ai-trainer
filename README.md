...
AI 기반 고객서비스 트레이닝 플랫폼 - 실전 음성 시뮬레이션과 AI 이론 평가를 통한 종합 교육 시스템

## 프로젝트 개요

CS AI 트레이너는 고객 서비스 상담원을 위한 종합 교육 플랫폼입니다. **실전 음성 시뮬레이션**과 **AI 이론 평가** 두 가지 모드를 제공하여 상담원의 실전 대응 능력과 이론 지식을 모두 향상시킬 수 있습니다. Google Cloud의 Speech-to-Text, Text-to-Speech, Gemini API를 활용하여 실제 고객과의 대화를 시뮬레이션하고, 학습 자료를 바탕으로 AI가 자동으로 시험 문제를 출제합니다.
...
5. **대화형 AI 고객**: Gemini API를 활용한 동적 고객 응답 생성
6. **AI 피드백**: Google Gemini API를 활용한 응대 내용 분석 및 피드백 제공

### 📝 AI 이론 평가
1. **학습 자료 관리**: 관리자가 Zip 파일(텍스트, PPTX 포함) 업로드
2. **자동 문제 출제**: Gemini AI가 학습 자료를 분석하여 객관식 + O/X 퀴즈 자동 생성
...
3. **통계 모니터링**: 전체 사용자 통계 확인

### 🎮 게임화
1. **레벨 시스템**: 점수에 따른 레벨업
2. **배지 시스템**: 다양한 업적 달성 시 배지 획득
3. **리더보드**: 전체 사용자 순위 경쟁
